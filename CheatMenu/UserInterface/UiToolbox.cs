...
namespace CheatMenu.UserInterface
{
	internal static class UiToolbox
	{
		...
		internal static void ChkMouseClick(Rect windowRect, ref bool uiBool)
		{
			LastMouseButtonUp = Input.GetMouseButtonUp(0) ? 0 : Input.GetMouseButtonUp(1) ? 1 : Input.GetMouseButtonUp(2) ? 2 : -1;

			if ((Input.mouseScrollDelta.y != 0 || LastMouseButtonUp >= 0) && IsMouseOnGUI(windowRect, ref uiBool))
			{
				Input.ResetInputAxes();
			}
		}

		internal static bool IsMouseOnGUI(Rect? windowRect, ref bool uiBool)
		{
			var point = new Vector2(Input.mousePosition.x, Screen.height - Input.mousePosition.y);
			return uiBool && windowRect.HasValue && windowRect.Value.Contains(point);
		}
