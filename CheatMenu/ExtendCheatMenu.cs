...
namespace CheatMenu
{
	...
	[BepInPlugin("ExtendCheatMenu", "ExtendCheatMenu", "1.3")]
	public class ExtendCheatMenu : BaseUnityPlugin
	{
		...
		public static ExtendCheatMenu Instance { get; private set; }
		...
		private void Awake()
		{
			...
			HotKeyEnabled = Config.Bind("HotKey", "1. Enable HotKey", false, "Use a hotkey to open ExtendCheatMenu.");
			HotKey = Config.Bind("HotKey", "2. HotKey", new KeyboardShortcut(KeyCode.C, KeyCode.LeftControl, KeyCode.LeftAlt), "HotKey to open ExtendCheatMenu with.");

			if (HotLoadDebug.debug)
            {
                DrawUi = true;
            } 
			else
			{
                SystemShortcutAPI.AddButton("ExtendCheatMenu", () =>
                {
                    DrawUi = !DrawUi;
                }, "Open ExtendCheatMenu UI", Convert.FromBase64String(Base64Icon));
            }
		}
